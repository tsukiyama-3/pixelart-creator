...
const init = () => {
  ...
  const imageData = uint32ArrayToImageData(pixels.value, pixelSize.value, pixelSize.value)
...
const onMouseleave = (event: MouseEvent) => {
  if (originalCoords.value) {
    // 元の色に戻す
    setPixelColor(originalCoords.value.x, originalCoords.value.y, originalColor.value!)
    renderPixel()
  }
  canvasLeave()
}
...
const renderPixel = () => {
  ...
  const imageData = offscreenContext?.createImageData(pixelSize.value, pixelSize.value)
...
const renderPreview = (context: CanvasRenderingContext2D | null | undefined) => {
...
const changeSize = (number: number) => {
  ...
  const originalPreviewScale = dotSize.value / ((pixelSize.value * dotSize.value) / previewSize.value)
            ...
            >
              <p
                ...
                :class="{
                  ...
                  'dark:text-[#2cb67d]': colorMode.preference === 'dark' && pixelSize !== 64,
                  'text-[#fffffe]': pixelSize === 64 && colorMode.preference === 'light',
                }"
              >
                64x64
              </p>
            ...
            >
              <p
                ...
                :class="{
                  ...
                  'dark:text-[#2cb67d]': colorMode.preference === 'dark' && pixelSize !== 32,
                  'text-[#fffffe]': pixelSize === 32 && colorMode.preference === 'light',
                }"
              >
                32x32
              </p>
            ...
            >
              <p
                ...
                :class="{
                  ...
                  'dark:text-[#2cb67d]': colorMode.preference === 'dark' && pixelSize !== 16,
                  'text-[#fffffe]': pixelSize === 16 && colorMode.preference === 'light',
                }"
              >
                16x16
              </p>
